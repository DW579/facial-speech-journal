...
function AnalyzeController($scope, $http, Analyze) {
  ...
  function handleSourceOpen(event) {
    ...
    sourceBuffer = mediaSource.addSourceBuffer('video/wmv; codecs="vp8"');
  ...
  function startRecording() {
    ...
    if (!MediaRecorder.isTypeSupported(options.mimeType)) {
      ...
      options = {mimeType: 'video/wmv;codecs=vp8'};
      if (!MediaRecorder.isTypeSupported(options.mimeType)) {
        console.log(options.mimeType + ' is not Supported');
        options = {mimeType: 'video/wmv'};
        if (!MediaRecorder.isTypeSupported(options.mimeType)) {
          console.log(options.mimeType + ' is not Supported');
          options = {mimeType: ''};
        }
  ...
  function play() {
    var superBuffer = new Blob(recordedBlobs, {type: 'video/wmv'});
    recordedVideo.src = window.URL.createObjectURL(superBuffer);
  }

  function download() {
    var blob = new Blob(recordedBlobs, {type: 'video/wmv'});
